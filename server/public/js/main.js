...
const statusLabel = (s) => {
  const map = {
    paid:    ['Payée',    'paid'],
    unpaid:  ['Impayée',  'unpaid'],
    overdue: ['En retard','overdue'],
    pending: ['Partielle','pending']
  };
...
const loadDashboard = async () => {
  try {
    // FIX: stats est direct { total, paid, ... }, overdue et actions ont .data
    const [stats, overdueRes, actionsRes] = await Promise.all([
      apiFetch('/invoice/stats'),
      apiFetch('/invoice/overdue'),
      apiFetch('/recovery/upcoming')
    ]);

    // stats retourne directement les champs (pas de .data)
    ...
    // FIX: overdue et upcoming retournent { success, data: [...] }
...
const renderInvoices = () => {
  ...
  if (!tbody) return;
  const list = invoiceFilter === 'all'
    ? invoicesData
    : invoicesData.filter(i => i.status === invoiceFilter);
  if (!list || list.length === 0) {
...
const loadRecovery = async () => {
  try {
    ...
    document.getElementById('rFailed').textContent     = stats.byOutcome?.failed      ?? 0;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (userNameEl && getToken()) {
    apiFetch('/user/me').then(u => {
      userNameEl.textContent = u.name || u.data?.name || 'Utilisateur';
    }).catch(() => {});
  }
